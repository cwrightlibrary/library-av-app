...
class AVApp(tk.Tk):
    def __init__(self, width: int = 1000, height: int = 625):
        ...
        if platform.system() == "Windows":
            ...
            self.app_icon = PhotoImage(file="src/icon.png")
    ...
    def setup_menubar(self):
        ...
        self.app_license = f"{self.app_name} v{self.app_version} is under the MIT license:"
        self.app_mit_license = mit_license(self.app_current_year, self.app_author, self.app_organization, self.app_sub_organization)
        ...
        self.HELP_MENU.add_command(label="About AV Issues", command=self.about_av_issues)
    ...
    def convert_csv(self):
        ...
        if file_path:
            try:
                ...
                converter = Converter()
                converter.excel_to_csv(excel_path=file_path.name, sheet_name="av_issues", csv_path=csv_file_path)
                self.df = pd.read_csv(csv_file_path)
                self.current_file_path = csv_file_path
                self.display_csv()
            except Exception as e:
                messagebox.showerror("Error", f"Unable to convert {file_path.name}. Error: {e}")
    ...
    def update_recent_files_menu(self):
        ...
        for file_path in self.recent_files:
            self.RECENT_MENU.add_command(label=file_path, command=lambda path=file_path: self.open_csv_from_recent(path))
    ...
    def display_csv(self):
        ...
        self.h_scrollbar = tk.Scrollbar(self.frame_issues, orient="horizontal", command=self.canvas.xview)
        self.v_scrollbar = tk.Scrollbar(self.frame_issues, orient="vertical", command=self.canvas.yview)
